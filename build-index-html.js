...
module.exports = function () {
    ...
    const chunks = slidesOrder.map(
        name => {
            const slideFile = path.resolve(__dirname, `./${lang}/${name}.html`);
            const content = fs.existsSync(slideFile)
                ? fs.readFileSync(path.resolve(__dirname, `./${lang}/${name}.html`), 'utf8')
                : `<section class="slide"><h1>slide-${name}</h1></section>`;
